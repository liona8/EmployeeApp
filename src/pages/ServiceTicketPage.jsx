        ...
        <div style={{
          ...
          <div>
            <div style={{
              ...
              <div style={{
                padding: "4px 10px", borderRadius: 100,
                background: pri.bg, fontSize: 11, fontWeight: 600, color: pri.color,
              }}>
                {ticket.priority}
              </div>
            </div>
        ...
        <div style={{ padding: "20px 24px", display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <div style={{ background: "var(--bg3)", border: "1px solid var(--border)", borderRadius: 10, padding: "14px 16px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              {[
                ["🏢 Building", ticket.location.building],
                ["🏬 Floor",    ticket.location.floor],
                ["🚪 Room",     ticket.location.room],
              ].map(([k, v]) => (
                <div key={k}>
...
function NewTicketModal({ onClose, onSubmit }) {
  const [form, setForm] = useState({
    ...
    room: "", floor: "", building: "HQ Tower",
  });
  const up = (k, v) => setForm(f => ({ ...f, [k]: v }));
  const valid = form.issue_title && form.description && form.room && form.floor;

  const handleSubmit = () => {
    if (!valid) return;
    const now = new Date().toISOString();
    onSubmit({
      id: `MT-2026-00${Math.floor(Math.random() * 90 + 10)}`,
      employee_id: "EMP001",
      issue_title: form.issue_title,
      description: form.description,
      category: form.category,
      priority: form.priority,
      location: { room: form.room, floor: form.floor, building: form.building },
      status: "Open",
      photo_url: null,
      created_at: now,
      updated_at: now,
      assigned_to: null,
      sla_hours: form.priority === "High" ? 4 : 8,
    });
    onClose();
  };
      ...
      <div style={{
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          ...
          <div className="form-group">
            ...
            <input className="form-input" placeholder="e.g. 5th Floor" value={form.floor} onChange={e => up("floor", e.target.value)} />
          </div>
        </div>
        <div className="form-group">
        ...
        <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 4 }}>
          ...
          <button className="btn btn-primary" onClick={handleSubmit} style={{ opacity: valid ? 1 : 0.45, cursor: valid ? "pointer" : "not-allowed" }}>
            Submit Ticket
          </button>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14, padding: "0 32px", marginBottom: 22 }}>
        {[
          ...
          <div
            ...
            onClick={() => setStatus(s.label === "In Progress" ? "In Progress" : s.label === filterStatus ? "All" : s.label)}
          >
            <div style={{
              position: "absolute", top: 0, left: 0, right: 0, height: 2,
              background: { red: "var(--danger)", orange: "var(--warning)", green: "var(--accent3)", gray: "var(--text3)" }[s.color],
            }} />
          ...
          {statuses.map(s => {
            ...
            return (
              <button
                ...
                onClick={() => setStatus(s)}
                style={{
                  background: filterStatus === s ? (sc?.bg || "rgba(91,124,250,0.12)") : "var(--bg2)",
                  border: `1px solid ${filterStatus === s ? (sc?.dot || "var(--accent)") : "var(--border)"}`,
                  color: filterStatus === s ? (sc?.color || "var(--accent)") : "var(--text2)",
              ...
              <div style={{ minWidth: 0 }}>
                ...
                <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
                  <span style={{ fontSize: 12, color: "var(--text3)" }}>
                    📍 {ticket.location.room}, {ticket.location.floor}
                  </span>
                  <span style={{ fontSize: 12, color: "var(--text3)" }}>
                    🏷 {ticket.category}
                  </span>
                  ...
                  {ticket.assigned_to && (
                    <span style={{ fontSize: 12, color: "var(--accent3)" }}>
                      👤 {ticket.assigned_to}
                    </span>
                  )}
                </div>
